...
**Status:** The desktop prototype and geodata visual rebuild are locally complete and verified; publication is being delivered through a GitHub PR. The live PR is authoritative for open/failed/merged state.
**Next action:** Run `gh pr view --json state,isDraft,mergeable,mergeStateStatus,statusCheckRollup,url`; if merged, continue with product/data work, otherwise resume the PR from its live state.
...
**Context pointers:** `README.md`, `DESIGN.md`, `components/MapScene.tsx`, `lib/matching.ts`, `public/map/manifest.json`, `data/official-data-snapshot.json`.
