...
export default function CommuteGuidePage() {
  ...
  return <>
    <JsonLd value={{ '@context': 'https://schema.org', '@type': 'Article', headline: metadata.title, description: metadata.description, datePublished: '2026-08-29', dateModified: '2026-09-07', mainEntityOfPage: url, publisher: { '@type': 'Organization', name: 'Where To BTO', url: absoluteUrl('/') } }} />
    <main className="content-main narrow-content">
      ...
      <article>
        <header className="content-hero"><h1>How to compare commutes before choosing a BTO</h1><p>A commute check should describe the complete weekly journey, not just the nearest station or distance to the city centre.</p><p className="data-line">Updated 7 Sep 2026 · 5 minute read</p></header>
        <div className="article-body">{sections.map(([title, body]) => <section key={title}><h2>{title}</h2><p>{body}</p></section>)}</div>
        <section className="content-section"><h2>Get a routed estimate for free</h2><p>Open a project in the explorer and use <strong>Check transit routes</strong>. The link opens Google Maps with the project and your selected workplace hub, or your custom pin. Choose a weekday departure time and check the walking legs and transfers. Pins are approximate anchors, so adjust the actual block entrance and workplace.</p><p>The estimate stays in Google Maps. We do not turn straight-line distance into invented minutes. Services and access routes may change before an uncompleted BTO is ready.</p><p><a href="https://www.onemap.gov.sg/">OneMap</a> also provides a public journey planner. For developers, <a href="https://www.onemap.gov.sg/apidocs/routing">OneMap routing</a> requires a registered token; it is separate from the credential-free Google Maps links in this tool.</p></section>
